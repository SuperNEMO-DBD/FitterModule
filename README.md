...
## Relevent Files:
- `SNFitter/
  - `DataProducts.h` : Definitions of the public data products generated by the algorithm
  - `SNFitter.{h,cpp}` : Fitting algorithm interface and implementation
  - `SNFitterImpl.{h,cpp}` : Private implementation details of `SNFitter`
- `SNFitterModule.cpp` : Falaise plugin module to use `SNFitter` in an `flreconstruct` pipeline
- `testing/`
  - Unit tests for `SNFitter` and `SNFitterImpl`
...
The build will create the `libSNFitter` shared library and `libSNFitterModule` flreconstruct plugin. Assuming that you have an `input.brio` file that contains a `TCD` bank (i.e. clustered data) from the reconstruction, this can be run after editing the configuration file to
point at the library location. No module configuration parameter are required.:

``` console

[name="flreconstruct.plugins" type="flreconstruct::section"]
plugins : string[1] = "SNFitterModule"
SNFitterModule.directory : string = "/my/path/to/build/directory"

# Define the modules in the pipeline:
[name="pipeline" type="SNFitterModule"]
