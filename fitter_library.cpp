...
std::vector<int> SNFitter::kink_finder(std::vector<double> betaangles, std::vector<double> errangles) {
...
LineFit SNFitter::fitline2D(std::vector<PathPoint> data) {
  ...
  lfxy->Eval();
  //  std::cout << "xy: i " << lfxy->GetParameter(0) << ", slxy " << lfxy->GetParameter(1) << std::endl;

  // fit in x-z plane
  lfxz->SetFormula("pol1");
  lfxz->AssignData(nd,1,xx,zz,ez);
  lfxz->Eval();
  //  std::cout << "xz: i " << lfxz->GetParameter(0) << ", slxz " << lfxz->GetParameter(1) << std::endl;
...
std::vector<BrokenLineFit> SNFitter::fitbrokenline() {
  ...
  for (auto& ppc : pathcollection) {
    ...
    //    cout << "n angles " << blf.angles.size() << " and n err angles " << errangles.size() << endl;
    // check
    ...
    if (peak_alarm(blf.angles)) {// otherwise nothing
      ...
      if (blf.breakpoints.empty()) {//  no kinks, empty
    	...
    	std::sort(blf.breakpoints.begin(), blf.breakpoints.end()); // is vector<int>, ascending order 
  ...
  std::sort(results.begin(), results.end(), [](BrokenLineFit& bl1, BrokenLineFit& bl2){return (bl1.chi2 < bl2.chi2);});
...
bool PathFinder::has_overlap(PathPoint p1, PathPoint p2) {
  if (p1.pointid != p2.pointid) {
    //    std::cout << "in overlap: x is " << overlap_x(p1, p2) << " and y is " << overlap_y(p1, p2) << std::endl;
...
bool PathFinder::is_neighbour(TrackerHit start, TrackerHit target) {
  ...
  if (ppstart.column == pptarget.column && ppstart.row == pptarget.row) // exclude itself as neighbour
    return false;

  // check grid x-y
  if (fabs(ppstart.column-pptarget.column) < 2 && fabs(ppstart.row-pptarget.row) < 2) // one grid place only
    return true;
  return false;
}
...
std::vector<int> PathFinder::column_hits(int col) {
  ...
  for (auto& hit : hits) {
    ...
    if (trmi.column == col) {
        ...
        // if (trmi.row == maxrow || trmi.row == minrow || trmi.row == maxrow-1 || trmi.row == minrow+1) {
  ...
  for (int hitid : found) { // find all nodes with that hit id
    ...
    columnpoints.insert(columnpoints.end(), temp.begin(), temp.end()); // all of temp into columnpoints
...
double PathFinder::edgelength(int e1, int e2) {
    ROOT::Math::XYZVector x0(ordered_points.at(e1).xc, ordered_points.at(e1).yc, ordered_points.at(e1).zc);
    ROOT::Math::XYZVector x1(ordered_points.at(e2).xc, ordered_points.at(e2).yc, ordered_points.at(e2).zc);
...
void PathFinder::clean_pointpairs(std::vector<std::pair<PathPoint, PathPoint> > im) {
  ...
  for (int nn : temp) {
	...
	if (has_overlap(onhost.at(where.at(which)), onhost.at(where.at(target)))) {// test all points
	    //std::cout << "merger on host = (" << onhost.at(where.at(which)).pointid.first << "," << onhost.at(where.at(which)).pointid.second << ") with (" 
	    //    << onhost.at(where.at(target)).pointid.first << "," << onhost.at(where.at(target)).pointid.second << ")" << std::endl;
	  mergehost.insert(std::make_pair(where.at(which),where.at(target))); // store, don't modify
	}
      }
    }
  }
  ...
  for (int nn : temp) {
	...
	if (has_overlap(onguest.at(where.at(which)), onguest.at(where.at(target)))) {// test all points
	    //std::cout << "merger on guest = (" << onguest.at(where.at(which)).pointid.first << "," << onguest.at(where.at(which)).pointid.second << ") with (" 
	   	//    << onguest.at(where.at(target)).pointid.first << "," << onguest.at(where.at(target)).pointid.second << ")" << std::endl;
	  mergeguest.insert(std::make_pair(where.at(which),where.at(target))); // store, don't modify
	}
      }
    }
  }
  ...
  //  std::cout << "size im " << im.size() << " with on host " << onhost.size() << " and on guest " << onguest.size() << " sizes " << std::endl;
  std::set<std::pair<int, size_t> > node_cleaner; // only unique entries
  for (unsigned int nn = 0; nn<im.size(); nn++) {
    related_points.push_back(std::make_pair(onhost.at(nn), onguest.at(nn))); // stored all valid point pairs
  ...
  for (auto& rp : related_points) {// store edges with indices
    ...
    // std::cout << "edges from id (" << nodes.at(pos1).first << "," << nodes.at(pos1).second << ") to id (" << nodes.at(pos2).first << "," << nodes.at(pos2).second << ")" << std::endl;
...
void PathFinder::make_edges() {
  ...
  for (counter=0; counter<nentries; counter++) {
    ...
    point[1] = y[counter];
    point[2] = z[counter];
    hittree->FindNearestNeighbors(point,nneighbours,indx,dist); // index needs to correspond to hit index
    for (int j=0;j<nneighbours;j++) {
      target = hits.at(indx[j]);
      if (is_neighbour(start, target)) {
	RelationalHit rh(start, target);
	//	std::cout << "pair made from " << start.mi.hitid << " and " << target.mi.hitid << std::endl;
	allpairs.push_back(rh); // RH has internal edges with own indices
      }
    }
  }
  
  //  std::cout << "all pairs made, done all relational hits, size = " << allpairs.size() << std::endl;
...
void RelationalHit::create_maps() {
  ...
  for (unsigned int i=0; i<allhost_errors.size() && !block; i++) { 
    ...
    if (it == merged.end()) {
      ...
      //      std::cout << "create map: host ival[" << ival.from() << ", " << ival.to() << "]" << std::endl;
      for (unsigned int j=i+1; j<allhost_errors.size(); j++) { // 
	Interval trial = allhost_errors.at(j);
	//	std::cout << "create map: host trial[" << trial.from() << ", " << trial.to() << "]" << std::endl;
	if (ival.angle_overlap(trial)) {
	  ...
	  // std::cout << "create map: overlap edge (" << i << ", " << i << ") inserted." << std::endl;
	  // std::cout << "create map: and edge (" << j << ", " << i << ") inserted." << std::endl;
	  Interval combined = ival.hull(trial); // merge tangent points and errors
	  //	  std::cout << "Hull interval: [" << combined.from() << ", " << combined.to() << "]" << std::endl;
	  // check for pihalf
	  int signx = 1;
	  ...
	  //	  std::cout << "hull bounds after pi half check: [" << start << ", " << end << "]" << std::endl;
	  if (sign1 < 0 && sign2 < 0) {
	    combined = Interval(start, end);
	    //	    std::cout << "New Hull interval: [" << combined.from() << ", " << combined.to() << "]" << std::endl;
	    signx = -1;
	  }
	  ...
	  ROOT::Math::XYVector relativetp(signx * rad * TMath::Cos(combiangle), signx * rad * TMath::Sin(combiangle)); // origin 0.0
	  ...
	  if (combined.to() - combiangle > TMath::Pi()/4.0) {
	    ...
	    ROOT::Math::XYVector dxdy = rphitoxy(tp, ROOT::Math::XYVector (host.gr.wirex, host.gr.wirey), rad, host.gr.rerr, combined.angle_dphi()); // returns dx, dy in vector
	    onthis.at(i) = std::make_pair(tp, dxdy); // same according to map
	    onthis.at(j) = std::make_pair(tp, dxdy);
	    //	    std::cout << "host - overlap edge (" << j << ", " << i << ") inserted." << std::endl;
	  }
	}
      }
      it = std::find(booked.begin(), booked.end(), i);
      if (it == booked.end()) { // store only once
	ROOT::Math::XYVector dxdy = rphitoxy(allhost_points.at(i), ROOT::Math::XYVector (host.gr.wirex, host.gr.wirey), host.gr.radius, host.gr.rerr, ival.angle_dphi()); // returns dx, dy in vector
	...
	//	std::cout << "host - create map: no overlap edge (" << i << ", " << i << ") inserted." << std::endl;
      }
    }
  }
  ...
  for (unsigned int i=0; i<allguest_errors.size() && !block; i++) {
    ...
    if (it == merged.end()) {
      ...
      //      std::cout << "create map: guest ival[" << ival.from() << ", " << ival.to() << "]" << std::endl;
      for (unsigned int j=i+1; j<allguest_errors.size(); j++) { // 
	Interval trial = allguest_errors.at(j);
	//	std::cout << "create map: guest trial[" << trial.from() << ", " << trial.to() << "]" << std::endl;
	if (ival.angle_overlap(trial)) {
	  merged.push_back(j); // not using this at later iteration
	  booked.push_back(i); // both i,j are replaced due to overlap
	  ...
	  // std::cout << "create map: and edge (" << j << ", " << i << ") inserted." << std::endl;
	  Interval combined = ival.hull(trial); // merge tangent points and errors
	  //	  std::cout << "guest Hull interval: [" << combined.from() << ", " << combined.to() << "]" << std::endl;
	  // check for pihalf
	  int signx = 1;
	  double start = combined.from();
	  double end = combined.to();
	  ...
	  if (sign1 < 0 && sign2 < 0) {
	    combined = Interval(start, end);
	    //	    std::cout << "guest New Hull interval: [" << combined.from() << ", " << combined.to() << "]" << std::endl;
	    signx = -1;
	  }
	  double combiangle = combined.angle_midinterval(); // into one node
	  //	  std::cout << " mid interval angle: " << combiangle << std::endl;
	  ...
	  if (combined.to() - combiangle > TMath::Pi()/4.0) {
	    ...
	    ROOT::Math::XYVector dxdy = rphitoxy(tp, ROOT::Math::XYVector (guest.gr.wirex, guest.gr.wirey), rad, guest.gr.rerr, combined.angle_dphi()); // returns dx, dy in vector
	    onother.at(i) = std::make_pair(tp, dxdy); // same according to map
	    onother.at(j) = std::make_pair(tp, dxdy); 
	    //	    std::cout << "guest - overlap edge (" << j << ", " << i << ") inserted." << std::endl;
	  }
	}
      }
      it = std::find(booked.begin(), booked.end(), i);
      if (it == booked.end()) { // store only once
	ROOT::Math::XYVector dxdy = rphitoxy(allguest_points.at(i), ROOT::Math::XYVector (guest.gr.wirex, guest.gr.wirey), guest.gr.radius, guest.gr.rerr, ival.angle_dphi()); // returns dx, dy in vector
	...
	//	std::cout << "guest - create map: no overlap edge (" << i << ", " << i << ") inserted." << std::endl;
      }
    }
  }
...
void RelationalHit::calculate_tangentpoints() {
  ...
  double distance = TMath::Sqrt((xwire1-xwire2)*(xwire1-xwire2) + (ywire1-ywire2)*(ywire1-ywire2));
...
void RelationalHit::calculate_overlapping_errors(double distance) {
  ...
  if (swapped) {
    ...
    dxdy.push_back(l_pairs.at(0).second - s_pairs.at(1).first); // l2-s3 since XYVectors can subtract
    ...
    dxdy.push_back(l_pairs.at(0).second - s_pairs.at(0).second); // l2-s2 since XYVectors can subtract
...
void RelationalHit::calculate_four(double distance) {
  ...
  // std::cout << "x1=" << TP_pair.first.x() << " y1=" << TP_pair.first.y() << " x2=" << TP_pair.second.x() << " y2=" << TP_pair.second.y() << std::endl;
  if (intern.order<1) { // host is host
    allhost_points.push_back(TP_pair.first);
    allhost_points.push_back(TP_pair.second);
  }
...
void RelationalHit::calculate_four_errors(double distance) {
  ...
  // std::cout << "four errors: rsmall=" << intern.rsmall << " rlarge=" << intern.rlarge << std::endl;
  ...
  if (intern.order) {
    ...
    if (swapped) {
      ...
      dxdy.push_back(l_pairs.at(1).second - s_pairs.at(0).first); // l4-s1 since XYVectors can subtract
    }
    else {
      dxdy.push_back(l_pairs.at(0).first  - s_pairs.at(0).first); // l1-s1 gives dx and dy
      dxdy.push_back(l_pairs.at(0).second - s_pairs.at(0).second); // l2-s2 since XYVectors can subtract
      dxdy.push_back(l_pairs.at(1).first  - s_pairs.at(1).first); // l3-s3 gives dx and dy
      dxdy.push_back(l_pairs.at(1).second - s_pairs.at(1).second); // l4-s4 since XYVectors can subtract
    }
  }
  else {
  ...
  if (ring_overlap >= 0.0) { // no origin between rings if they overlap
    intersectioncalc(distance);
    l_pairs.push_back(TP_pair); // l1, l2 in here
    l_pairs.push_back(std::make_pair(TP_pair.second, TP_pair.first)); // swapped order of tp, l3, l4
  }
  else {
...
void RelationalHit::equal_rings(double distance, bool oflag) {
  ...
  ROOT::Math::XYVector tp1(intern.xa + intern.rsmall*TMath::Sin(alpha), intern.ya - intern.rsmall*TMath::Cos(alpha));
  ROOT::Math::XYVector tp2(intern.xa - intern.rsmall*TMath::Sin(alpha), intern.ya + intern.rsmall*TMath::Cos(alpha));
  ...
  ROOT::Math::XYVector tp3(intern.xb + intern.rlarge*TMath::Sin(alpha), intern.yb - intern.rlarge*TMath::Cos(alpha));
  ROOT::Math::XYVector tp4(intern.xb - intern.rlarge*TMath::Sin(alpha), intern.yb + intern.rlarge*TMath::Cos(alpha));
...
void RelationalHit::intersectioncalc(double distance) {
  ...
  double d = TMath::Sqrt(fabs(distance*distance*(2.0*intern.rlarge*intern.rlarge + 2.0*intern.rsmall*intern.rsmall - distance*distance) - dummy*dummy));
  double p1 = (intern.yb-intern.ya)*d / (2*distance*distance);
  double p2 = (intern.xb-intern.xa)*d / (2*distance*distance);
  double x_1 = 0.5*(intern.xa+intern.xb) - ((intern.xb-intern.xa)*dummy) / (2.0*distance*distance) + p1;
  double x_2 = x_1 - 2.0*p1;
  double y_1 = 0.5*(intern.ya+intern.yb) - ((intern.yb-intern.ya)*dummy) / (2.0*distance*distance) - p2;
...
void RelationalHit::calc_onvertical(double xw, double yw, double rad, double h, double xo, double yo) {
...
void RelationalHit::calc_onhorizontal(double xw, double yw, double rad, double h, double xo, double yo) {
...
void RelationalHit::pointcalc(double xw, double yw, double rad, double h, double xo, double yo) {
  ...
  if (fabs(yw-yo) <= EPS) {
    ...
    double y_1 = (xw*yw*xo - x_1*(xo*yw + xw*(yw-2.0*yo)) + x_1*x_1*(yw-yo) + rad*rad*yo - xw*xw*yo) / denom1;
    double y_2 = (xw*yw*xo - x_2*(xo*yw + xw*(yw-2.0*yo)) + x_2*x_2*(yw-yo) + rad*rad*yo - xw*xw*yo) / denom2;
...
ROOT::Math::XYVector RelationalHit::rphitoxy(ROOT::Math::XYVector tp, ROOT::Math::XYVector ctr, double r, double dr, double dphi) {
...
std::vector<Interval> RelationalHit::deltaxy_to_deltaphi(std::vector<ROOT::Math::XYVector> xy, std::vector<ROOT::Math::XYVector> dxdy, std::vector<ROOT::Math::XYVector> ctr) {  // conversion to error on ring
  std::vector<Interval> dphi;
  int counter = 0;
  for (auto& dublet : xy) {
    double p = TMath::ATan2(dublet.y() - ctr.at(counter).y(), dublet.x() - ctr.at(counter).x());
    // std::cout << "dxy to dphi: point and centre " << dublet.x() << " " << dublet.y() << "; " << ctr.at(counter).x() << " " << ctr.at(counter).y() << std::endl;
    // std::cout << "dxy to dphi: angle to point " << p << std::endl;
    ...
    Interval ival(p1, p2);
    dphi.push_back(ival);
    //    std::cout << "dxy to dphi: interval[" << ival.from() << ", " << ival.to() << "]" << std::endl;
    counter++;
  }
...
double Interval::angle_midinterval() {
  ...
  if (signleft<0 && signright<0) {
    ...
    //    std::cout << "midint - after check for pi half: [" << left << ", " << right<< "]" << std::endl;
...
double Interval::angle_dphi() {
  ...
  if (signright<0 && signleft<0) {
    //    std::cout << "dphi - after check for pi half: " << right << " - " << midpoint << std::endl;
...
bool Interval::angle_overlap(Interval other)
{
  ...
  // if (swapped2)
  //   std::cout << "after check 2 for pi half: [" << otherleft << ", " << otherright << "]" << std::endl;
