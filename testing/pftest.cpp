...
std::vector<PathPointCollection> testPFbl() {
  ...
  double rad[10] = {11.8831, 9.96064, 6.39051, 15.4532, 0.897902, 20.9459, 17.2491, 4.5947, 11.7564, 11.2643};
...
TEST_CASE( "PF A", "[falaise][line3p]" ) {
  REQUIRE( check_linepath() == false );
}


TEST_CASE( "PF B", "[falaise][triangle]" ) {
  REQUIRE( check_trianglepath() == 23 );
}

TEST_CASE( "PF C", "[falaise][simpath]" ) {
  REQUIRE( check_simpath() == 6 );
}

TEST_CASE( "PF D", "[falaise][blpath]" ) {
  REQUIRE( check_blpath() == false );
}
